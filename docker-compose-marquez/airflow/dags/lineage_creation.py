...
with DAG(
      ...
      try: 
        ...
        for key in dictionary_task:
          if dictionary_task[key] == False:
            ...
            loop.run_until_complete(lineage_creation_task(key, None, None, producer, **context))
          else:
    ...
    def table_lineage(**context):
      try:
        ...
        for namespace in namespaces["namespaces"]:
          #Get all tasks
          try:
            jobs = requests.get(marquez_backend + "namespaces/" + namespace["name"] + "/jobs").json()
        ...
        for key in dictionary_table:
          if dictionary_table[key] == False:
            ...
            loop = asyncio.get_event_loop()
            loop.run_until_complete(lineage_creation_table(key, producer, **context))
          else:
    ...
    async def lineage_creation_task(parent_dag_id, next_task_ids, wait_for_completion, producer, **context):
        dictionary_task = context['ti'].xcom_pull(key='dictionary_task')
        ...
        for task in dag["tasks"]:
            ...
            if task["operator_name"] == "TriggerDagRunOperator":
                ...
                downstream_roots = await lineage_creation_task(target_dag_id, task["downstream_task_ids"], wait_for_completion_downstream, producer, **context)
        ...
        for node in graph:
            ...
            if node["type"] == "DATASET":
                for edge in node["inEdges"]:
                    ...
                    if node_type == "Spark":
                        await create_spark_task_dataset_relationship(origin, node_job_name, producer)
                    if node_type == "Airflow":
                        await create_task_dataset_relationship(origin, node_job_name, producer)
                    else:
                        print("Unresolved")
            elif node["type"] == "JOB":
                ...
                for edge in node["inEdges"]:
                    origin = extract_job_name(edge["origin"])
                    if node_type == "Spark":
                        await create_dataset_spark_task_relationship(origin, destination, producer)
                    elif node_type == "Airflow":
                        await create_dataset_task_relationship(origin, destination, producer)
                    else:
                        print("Unresolved")
        context['ti'].xcom_push(key='dictionary_table', value=dictionary_table)
    ...
    table_lineage = PythonOperator(task_id='table_lineage', python_callable=table_lineage)
    ...
    #       task_details = requests.get(marquez_backend + "namespaces/example/jobs/" + dag_id + "." + task["task_id"]).json()
    #       if task["operator_name"] == "TriggerDagRunOperator":
    #         target_dag_id = task_details["latestRun"]["facets"]["airflow"]["task"]["args"]["trigger_dag_id"]
    #         dummy_recursive_function(target_dag_id, **context)
    #   except requests.exceptions.RequestException as err:
